	...
	Serial(char* port_name)
	{
		...
		if(m_com_port == INVALID_HANDLE_VALUE)
		{
			if(GetLastError() == ERROR_FILE_NOT_FOUND)
				moony::logError() << "Handle was not attached. Reason: %s not available." << port_name;
			else
				moony::logError() << "Unkown Error";
		}
		else
		{
			DCB serial_args = {0};
			...
			if(!GetCommState(m_com_port, &serial_args))
				moony::logError() << "Failed to get current serial parameters!";
			else
			{
				...
				if(!SetCommState(m_com_port, &serial_args))
					moony::logDebug() << "Could not set Serial Port parameters";
				else
				{
					m_connected = true;
